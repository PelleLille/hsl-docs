...
At multiple stages in the e-mail delivery process there are opportunities to affect the behavior (e.g. accept or reject an e-mail message). These are referred to as `flows`, `scripts` or `modules`, each of which extends the language with additional functions and pre-defined variables.
